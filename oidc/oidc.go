...
func (c OIDCConfig) NewClient() (*OIDCClient, error) {
	...
	if c.ClientSecret == "" && !c.UsePKCE {
		return nil, errors.New("client secret is empty and PKCE is disabled")
	}
	...
	if c.CookieHandler != nil {
		...
		cookieHandler = cookie.NewCookieHandler(c.Secret, c.Secret, cookie.WithDomain(baseURL.Hostname()), cookie.WithPath(baseURL.Path), cookie.WithSameSite(http.SameSiteLaxMode), cookie.WithMaxAge(int(c.SessionLength.Seconds())))
	}
...
func (c OIDCClient) getCookieName() string {
	return c.client.OAuthConfig().ClientID + "-session"
}
...
func (c OIDCClient) NewLoginHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil || !path.IsAbs(redirect.Path) {
...
func (c OIDCClient) NewLogoutHandler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		sessionCookie, err := r.Cookie(c.getCookieName())
